...
export default function DashboardView({ 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel p-8 bg-gradient-to-r from-[#121720] via-[#090c10] to-[#121720] border border-[#e39e2e]/30 relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 relative z-10">
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              <span className="badge badge-gold font-mono">Operations Command Center</span>
              <span className="text-xs text-[#a7b0c0] font-mono">Date: {dailyBatch.date}</span>
            </div>
            <h2 className="text-3xl font-black text-[#eaf2ff] tracking-tight uppercase">
              Yaga Calls Multi-Creator Distribution Engine
            </h2>
            <p className="text-sm text-[#a7b0c0] max-w-3xl leading-relaxed">
              Automated 3-batch Telegram dispatch hub managing <span className="text-white font-bold">{creators.length} Creators</span> across <span className="text-white font-bold">{platforms.length} Target Platforms</span> with zero link submission friction.
            </p>
          </div>

          <div className="flex items-center gap-4 bg-[#090c10] p-4 rounded-2xl border border-white/10 shrink-0">
            <div className="text-right">
              <div className="text-[10px] text-[#a7b0c0] uppercase font-bold tracking-widest">System Phase</div>
              <div className="text-sm font-extrabold text-white flex items-center gap-1.5 justify-end mt-0.5">
            ...
            <button
              ...
              className="grad-button px-5 py-2.5 rounded-xl text-xs font-black shadow-lg cursor-pointer flex items-center gap-2"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="glass-card-interactive p-6 space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-[#a7b0c0] uppercase tracking-wider">Onboarding Gate</span>
            <div className="w-9 h-9 rounded-xl bg-[#00d294]/15 border border-[#00d294]/30 flex items-center justify-center text-[#00d294]">
              <ShieldCheck className="w-5 h-5" />
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-4xl font-black text-white">{activeAccountsCount}</span>
        ...
        <div className="glass-card-interactive p-6 space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-[#a7b0c0] uppercase tracking-wider">Today's Dispatch</span>
            <div className="w-9 h-9 rounded-xl bg-[#e39e2e]/15 border border-[#e39e2e]/30 flex items-center justify-center text-[#e39e2e]">
        ...
        <div className="glass-card-interactive p-6 space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-[#a7b0c0] uppercase tracking-wider">Active Team</span>
            <div className="w-9 h-9 rounded-xl bg-[#38bdf8]/15 border border-[#38bdf8]/30 flex items-center justify-center text-[#38bdf8]">
              <Users className="w-5 h-5" />
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-4xl font-black text-white">{creators.length}</span>
        ...
        <div className="glass-card-interactive p-6 space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-[#a7b0c0] uppercase tracking-wider">Support Desk</span>
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-4xl font-black text-white">{openIssues.length}</span>
          ...
          <div className="text-xs font-bold">
            {openIssues.length > 0 ? (
              <span className="text-rose-400 cursor-pointer hover:underline flex items-center gap-1" onClick={() => onNavigateTab('issues')}>
                Requires Owner Attention <ArrowUpRight className="w-3.5 h-3.5" />
              </span>
            ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-panel p-6 space-y-6">
          <div className="flex items-center justify-between pb-4 border-b border-white/10">
            <div>
              <h3 className="text-lg font-black text-white flex items-center gap-2 uppercase tracking-tight">
                <Clock className="w-5 h-5 text-[#e39e2e]" />
                Staggered 3-Batch Telegram Distribution Engine
              </h3>
              <p className="text-xs text-[#a7b0c0]">
                Daily posts partitioned into 3 equal batches (1/3rd per batch) dispatched every 30 minutes to all creators.
              </p>
            </div>
            <button 
              onClick={() => onNavigateTab('studio')}
              className="px-4 py-2 rounded-xl bg-[#e39e2e]/10 border border-[#e39e2e]/35 hover:bg-[#e39e2e]/20 text-[#e39e2e] text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[1, 2, 3].map((batchNum) => {
              ...
              return (
                <div key={batchNum} className={`p-5 rounded-2xl bg-[#090c10] border border-${borderColor} space-y-3 relative overflow-hidden`}>
                  <div className="flex items-center justify-between">
                    <span className={`badge ${badgeClass}`}>Batch {batchNum} ({offsetLabel})</span>
          ...
          <div className="space-y-3 pt-3">
            ...
            <div className="space-y-2.5">
              {dailyBatch.tasks.slice(0, 4).map((task) => (
                <div key={task.id} className="p-4 rounded-xl bg-[#090c10] border border-white/10 flex items-center justify-between text-xs">
                  <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-[#a7b0c0] font-mono">{task.publishTime}</span>
                    <span className={`px-3 py-1 rounded-lg text-xs font-extrabold ${
                      ...
                      {task.status === 'Completed' ? '[Mark as Done]' : 'Pending'}
                    </span>
                  </div>
                </div>
              ))}
        ...
        <div className="glass-panel p-6 space-y-4">
          ...
          <div className="space-y-3.5">
            {creators.map((c) => (
              <div key={c.id} className="p-4.5 rounded-2xl bg-[#090c10] border border-white/10 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-[#e39e2e] to-[#d5b895] flex items-center justify-center font-black text-[#090c10] text-sm shadow-md">
                      {c.publicName.substring(0, 2).toUpperCase()}
                    </div>
                    <div>
                      <div className="text-sm font-bold text-white">{c.publicName}</div>
                      <div className="text-xs text-[#a7b0c0] font-medium">{c.title}</div>
                    </div>
                  </div>
                  <span className="badge badge-emerald text-[10px]">Active</span>
                </div>
                <div className="text-xs text-slate-200 bg-[#121720] p-3 rounded-xl border border-white/5 leading-relaxed font-sans">
                  <span className="text-[#a7b0c0] font-semibold text-[11px] block uppercase mb-0.5">Voice Style:</span>
                  {c.voiceProfile.tone}
                </div>
              </div>
            ))}
          ...
          <button
            ...
            className="w-full py-3 rounded-xl bg-[#121720] hover:bg-[#19202d] text-white text-xs font-bold transition-all border border-white/10 cursor-pointer shadow-md"
          >
            Manage Creators & Credential Vault ➔
          </button>
        </div>
      </div>
    </div>
